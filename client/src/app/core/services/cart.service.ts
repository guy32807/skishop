...
export class CartService {
  ...
  totals = computed(() => {
    ...
    return { subtotal, shipping, tax, discountAmount, total};
  });
  ...
  setCart(cart: Cart) {
    this.cart.set(cart); // Update signal immediately for instant UI update
    return this.http.post<Cart>(this.baseUrl + 'cart', cart).subscribe({
      next: (updatedCart) => {
        this.cart.set(updatedCart); // Update with server response
      },
      error: (error) => {
